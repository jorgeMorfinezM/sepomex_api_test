...
class UsersAuth(Base):
    ...
    Transactions:
     - Insert: Add ciudad data to the database if not exists.
     - Update: Update ciudad data on the database if exists.
    """
...
# Transaction to looking for a ciudad on db to authenticate
def validate_user_exists(user_name):
    r"""
    Looking for a ciudad by name on the database to valid authentication.

    :param user_name: The ciudad name to valid authentication on the API.
    :return result: Boolean to valid if the ciudad name exists to authenticate the API.
    """
...
# Transaction to update ciudad' password  hashed on db to authenticate
def update_user_password_hashed(user_name, password_hash):
    r"""
    Transaction to update password hashed of a ciudad to authenticate on the API correctly.

    :param user_name: The ciudad name to update password hashed.
...
def insert_user_authenticated(user_id, user_name, user_password, password_hash):
    r"""
    Transaction to add a ciudad data to authenticate to API, inserted on the db.

    :param user_id: The Id of the ciudad to add on the db.
    :param user_name: The ciudad name of the ciudad to add on the db.
    :param user_password:  The password od the ciudad to add on the db.
    :param password_hash: The password hashed to authenticate on the API.
    """
...
def get_data_user_authentication(session, table_name, user_name):
    ...
    try:
        ...
        for ciudad in user_auth_db:
            if ciudad is not None:

                user_name_db = ciudad['username']
                user_password_db = ciudad['password']
                password_hash = ciudad['password_hash']
                last_update_date = datetime.strptime(str(ciudad['last_update_date']), "%Y-%m-%d")
